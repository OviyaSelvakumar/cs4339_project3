...
Promise.all(removePromises)
  .then(() => {
    ...
    const allPromises = Promise.all(userPromises).then(() => {
      ...
      return Promise.all(photoPromises).then(() => SchemaInfo.create(models.schemaInfo2()).then(() => { console.log('SchemaInfo object created'); }) // Create a single SchemaInfo document (no version field required)
        .catch((err) => {
